...
essential = ["uid", "password", "remote_addr", "remote_platform", "remote_platform_version", "remote_addr", "remote_platform", "remote_platform_version", "salt", "_"]
keys = ["uid", "password", "remote_addr", "remote_platform", "remote_platform_version", "remote_addr", "remote_platform", "remote_platform_version", "salt", "_"]
nullable = []
validation_function = {
    ...
    "_": lambda x: validate_str(x, 1, 128)
}
...
def find_user(data):
    ...
    print("data.get(_) : {}".format(data.get("_", None)))
    ...
    user.password = make_hashed(encryption_password(data.get("_", None)) + new_salt)
    user.salt = encryption_salt(new_salt)
    print("new password : {}".format(user.password))
    print("new salt : {}".format(user.salt))
